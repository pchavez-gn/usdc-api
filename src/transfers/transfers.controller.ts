...
export class TransfersController {
  ...
  @Post('transfer')
  @ApiOperation({
    summary: 'Simulate a USDC transfer (demo only, no real transaction)',
  })
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        ...
        amount: {
          type: 'string',
          description: 'Amount of USDC to transfer (as string, e.g., "10.5")',
        },
      },
      required: ['fromPk', 'to', 'amount'],
    },
  })
  async simulateTransferDemo(
    @Body() body: { fromPk: string; to: string; amount: string },
  ) {
